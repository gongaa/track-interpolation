...
class IOULoss(torch.nn.Module):
    ...
    def forward(self, gt_bbox, gen_bbox, update=True):
        # (bs, num_track, 4)
        gt = gt_bbox.view(-1, 4)
        gen = gen_bbox.view(-1, 4)
        xA = torch.max(gt[:,0], gen[:,0])
        yA = torch.max(gt[:,1], gen[:,1])
        xB = torch.min(gt[:,2], gen[:,2])
        yB = torch.min(gt[:,3], gen[:,3])
        interArea = torch.mul(torch.max(torch.zeros_like(xB), xB-xA+1), torch.max(torch.zeros_like(yB), yB-yA+1))
        boxAArea = torch.mul(gt[:,2]-gt[:,0], gt[:,3]-gt[:,1])
        boxBArea = torch.mul(gen[:,2]-gen[:,0], gen[:,3]-gen[:,1])
        iou = interArea.float() / (boxAArea+boxBArea-interArea).float()
        return iou    # bs*num_track
...
class ObjLoss(torch.nn.Module):
    def __init__(self, args, window_size = 11, size_average = True, prefix='obj_'):
    ...
    def forward(self, input, gt):
        l1_loss = self.l1_loss(input, gt)
        vgg_loss = self.vgg_loss(input, gt, False)
        ssim_loss= self.ssim_loss(input, gt) 
        gdl_loss = self.gdl_loss(input, gt)
        return  OrderedDict( 
                [('{}l1_loss'.format(self.prefix), self.args.obj_l1_weight*l1_loss), 
                ('{}gdl_loss'.format(self.prefix), self.args.obj_gdl_weight*gdl_loss), 
                ('{}vgg_loss'.format(self.prefix), self.args.obj_vgg_weight*vgg_loss), 
                ('{}ssim_loss'.format(self.prefix),self.args.obj_ssim_weight*ssim_loss)]
                )
